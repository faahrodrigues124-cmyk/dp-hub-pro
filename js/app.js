...
function calcularEngineGabarito() {
    ...
    // Comissões também geram reflexo de DSR para o comissionista.
    // A base didática da bancada soma HE + adicional noturno + comissão.
    const totDsr = diasUteis > 0 ? ((totHE50 + totAdn + comissao) / diasUteis) * dsrDias : 0;
...
function carregarCasoIntegrado(nivel) {
    ...
    const dsrVariáveis = ((vHE50 + vNoturno + comissaoSorteada) / diasUteis) * dsrDias;
...
function conferirExercicioBancada() {
    ...
    if (abaAtivaAtual === 'folha') {
        ...
        const dsr = ((heTotal + adn + comissao) / 24) * 6;
