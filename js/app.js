...
function calcularEngineGabarito() {
    if (!casoAtualGerado) return {};
    const salario = casoAtualGerado.salario;
    let valAdicional = 0;
    if (casoAtualGerado.tipoAdicional === 'periculosidade') valAdicional = salario * 0.30;
    else if (casoAtualGerado.tipoAdicional === 'insalubridade_20') valAdicional = ENGINE_DP.SALARIO_MINIMO_2026 * 0.20;

    const vHora = (salario + valAdicional) > 0 ? (salario + valAdicional) / 220 : 0;
    const vHE50Unit = vHora * 1.50;
    const diasUteis = 24, dsrDias = 6;

    const noturnoRelogio = casoAtualGerado.noturnoRelogio;
    const noturnoReduzido = noturnoRelogio * (60 / 52.5);
    const vAdcNoturnoHora = vHora * 0.20;
    const totAdn = noturnoReduzido * vAdcNoturnoHora;

    const qtdHE50 = casoAtualGerado.qtdHE50;
    const totHE50 = qtdHE50 * vHE50Unit;
    const totDsr = ((totHE50 + totAdn) / diasUteis) * dsrDias;
    const comissao = casoAtualGerado.comissao;
    const totProventos = salario + valAdicional + totHE50 + totAdn + totDsr + comissao;

    const descFaltas = casoAtualGerado.descFaltas;
    const descAtrasos = 0, descVT = 0;

    const baseINSS = Math.max(0, totProventos - descFaltas);
    const descINSS = ENGINE_DP.calcularINSS(baseINSS);
    const numDeps = casoAtualGerado.numDeps;
    const descIRRF = ENGINE_DP.calcularIRRF(baseINSS, descINSS, numDeps);
    const totDescontos = descFaltas + descINSS + descIRRF;
...
function validarCampos() {
    ...
    mapaCampos.forEach(c => {
        ...
        if (!el || gabarito[c.key] === undefined) return;
...
function gerarNovoCasoBancada() {
    const salarios = [3000, 3500, 4200, 4800, 5500];
    const salario = salarios[Math.floor(Math.random() * salarios.length)];
    const tiposAdic = ['nenhum', 'periculosidade', 'insalubridade_20'];
    const tipoAdicional = tiposAdic[Math.floor(Math.random() * tiposAdic.length)];
    const he = Math.floor(Math.random() * 10) + 6;
    const noturno = Math.floor(Math.random() * 20) + 10;
    const comissao = Math.random() > 0.5 ? Math.floor(Math.random() * 800) + 200 : 0;
    const faltas = Math.random() > 0.6 ? 1 : 0;
    const deps = Math.floor(Math.random() * 3);

    casoAtualGerado = {
        salario, tipoAdicional, qtdHE50: he, noturnoRelogio: noturno, comissao,
        descFaltas: faltas > 0 ? (salario / 30) * 2 : 0, numDeps: deps
    };

    let adicNome = 'Nenhum';
    if (tipoAdicional === 'periculosidade') adicNome = 'Periculosidade (30%)';
    else if (tipoAdicional === 'insalubridade_20') adicNome = 'Insalubridade Média (20%)';

    document.getElementById('bancada-caso-enunciado').innerHTML = `
        <b>Cenário de Treino Integrado:</b><br>
        • Salário Base: <b>R$ ${formatBRL(salario)}</b> (220h mensais, 24 dias úteis, 6 DSR).<br>
        • Adicional: <b>${adicNome}</b>.<br>
        • Variáveis: <b>${he}h Extras (50%)</b> | <b>${noturno}h Noturnas (relógio)</b> | Comissões: <b>R$ ${formatBRL(comissao)}</b>.<br>
        • Ocorrências: <b>${faltas} falta injustificada</b> (com desconto de DSR) | Dependentes IRRF: <b>${deps}</b>.<br>
        <i>Preencha os campos abaixo com base neste cenário.</i>
    `;

    document.getElementById('wb-salario').value = formatBRL(salario);
    document.getElementById('wb-tipo-adicional').value = tipoAdicional;
    document.getElementById('wb-qtd-he50').value = he;
    document.getElementById('wb-noturno-relogio').value = noturno;
    document.getElementById('wb-comissao').value = formatBRL(comissao);
    document.getElementById('wb-desc-faltas').value = formatBRL(casoAtualGerado.descFaltas);
    document.getElementById('wb-num-deps').value = deps;

    validarCampos();
    showToast('Novo cenário gerado com sucesso!');
}

function copiarCasoEExercicio() {
    const enunciado = document.getElementById('bancada-caso-enunciado').innerText;
    navigator.clipboard.writeText(enunciado).then(() => showToast('Enunciado copiado para envio!'));
}
...
function calcPercent() {
    try {
        ...
        if (parts.length >= 3) {
            ...
            calcDisplay.innerText = res.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
